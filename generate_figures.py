...
def main():
    ...
    #draw_style_mixing_figure(os.path.join(config.result_dir, 'style-mixing-pretrained.png'), load_Gs(url_mine), w=512, h=384, src_seeds=[5,14,62], dst_seeds=[122,139,147], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,14)])
    draw_style_mixing_figure(os.path.join(config.result_dir, 'style-mixing-our.png'), load_Gs(url_mine), w=256, h=256, src_seeds=[155,110,95], dst_seeds=[80,85,25], style_ranges=[range(0, 4)] * 3 + [range(4, 8)] * 2 + [range(8, 14)])
